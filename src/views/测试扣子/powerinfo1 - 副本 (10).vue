    ...
    <div class="row-social">
      <div class="social-tile" v-for="item in socialItems" :key="item.label">
        <div class="tile-icon-wrap" :style="{ backgroundColor: item.iconBg }">
          <el-icon v-if="item.icon" :size="22" :color="item.iconColor"><component :is="item.icon" /></el-icon>
        </div>
        <div class="tile-body">
...
import {
  ...
  CaretTop, CaretBottom, Minus, Sunrise, Location
...
const socialItems = computed(() => {
  ...
  return [
    ...
    { label: '绿色发电', display: '972.5', unit: '万kWh', icon: null, iconBg: 'rgba(64,158,255,0.12)', iconColor: '#409eff', context: `相当于 ${Math.round(totalKWh/10000/2000*5)} 户家庭年用电` },
  ]
})
