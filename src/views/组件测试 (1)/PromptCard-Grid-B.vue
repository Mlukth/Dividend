  ...
  <div class="prompt-card-container">
    ...
    <header class="relic-bar">
      <div class="relic-left">
        ...
        <span class="scheme-badge">方案B - 渐变描边</span>
      </div>
...
const getCategoryName = (category) => {
  const names = { CDEV: '代码工程', PM: '提示词管理', INFO: '信息处理', LEARN: '学习辅助', DOC: '文书产出', TOOL: '工具开发' }
...
onMounted(() => {
  mockCards.forEach(card => {
    ...
    if (saved) card.usageCount = parseInt(saved)
  })
})
</script>
...
.scheme-badge {
  font-size: 11px;
  padding: 4px 10px;
  background: rgba(203,106,82,0.2);
  color: #CB6A52;
  border-radius: 20px;
  border: 1px solid rgba(203,106,82,0.3);
}
...
/* 方案B：渐变描边效果层 */
.prompt-card.rarity-copper .gradient-border-effect {
  position: absolute;
  inset: 0;
  border-radius: 12px;
  padding: 1px;
  background: linear-gradient(
    135deg,
    rgba(203,106,82,0.4) 0%,
    rgba(203,106,82,0.1) 25%,
    rgba(203,106,82,0.05) 50%,
    rgba(203,106,82,0.1) 75%,
    rgba(203,106,82,0.4) 100%
  );
  -webkit-mask: 
...
/* 悬浮时边框增强 */
.prompt-card.rarity-copper:hover .gradient-border-effect {
  opacity: 1;
  animation: borderGlowIntense 2s ease-in-out infinite;
}
...
.prompt-card.rarity-gold {
  ...
  box-shadow: 0 0 20px rgba(197,171,80,0.2), inset 0 1px 0 rgba(255,255,255,0.3);
}
...
.prompt-card.rarity-silver .card-name { color: #fff; }
.prompt-card.rarity-bronze .card-desc,
.prompt-card.rarity-silver .card-desc { color: rgba(255,255,255,0.75); }
.prompt-card.rarity-bronze .card-code,
.prompt-card.rarity-silver .card-code { color: rgba(255,255,255,0.5); }
...
.detail-card.rarity-silver { background: linear-gradient(135deg, rgba(169,113,66,0.1) 0%, transparent 100%); }
