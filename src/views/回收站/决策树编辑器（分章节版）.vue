<template>
  <div class="tree-editor-page">
    ...
    <div class="left-panel">
      ...
      <!-- 章节切换器 -->
      <div v-if="chapterList.length > 0" class="section-card chapter-card">
        <div class="section-title">
          <el-icon><Collection /></el-icon> 章节切换
        </div>
        <el-select v-model="currentChapterId" placeholder="选择章节" style="width: 100%">
          <el-option
        ...
        <div class="chapter-info">
          当前章节节点数：{{ currentSubtree.length }} 个
        </div>
      </div>
          ...
          placeholder='[
  {
    ...
    ...
        ...
        <div class="import-stats">
          <span>总节点数：{{ localTree.length }} 个</span>
        </div>
      </div>
    ...
    <div class="right-panel">
      ...
      <div class="editor-body">
        <div class="canvas-container">
          <div v-if="currentSubtree.length === 0" class="tree-empty">
            <el-empty description="当前章节尚无节点，请添加根节点或使用 AI 生成" />
          </div>
...
import {
  ...
  FolderOpened, Upload, Download, Collection
...
const chapterList = computed(() => {
  return localTree.value.filter(n => n.parentId === null)
})

// 当前章节的完整子树节点（用于画布显示）
const currentSubtree = computed(() => {
  if (!currentChapterId.value) return []
  const root = localTree.value.find(n => n.id === currentChapterId.value)
  if (!root) return []
  const result = []
  const stack = [root.id]
  while (stack.length) {
    const id = stack.pop()
    const node = localTree.value.find(n => n.id === id)
    if (node) {
      result.push(node)
      const children = localTree.value.filter(n => n.parentId === id)
...
// 根据当前章节重建 VueFlow 节点与连线
const rebuildTreeFlow = () => {
  const subtree = currentSubtree.value
  if (subtree.length === 0) {
    treeFlowNodes.value = []
    treeFlowEdges.value = []
    return
  ...
  // 为这些节点布局：纵向，从上到下
  const layoutSubtree = (rootId, x, startY) => {
    const positions = {}
    const process = (nodeId, x, y) => {
      positions[nodeId] = { x, y }
      const children = subtree.filter(n => n.parentId === nodeId)
      const gap = 100  // 子节点垂直间距
      const childYStart = y + 120  // 父节点与子节点的纵向距离
      let currentY = childYStart - ((children.length - 1) * gap) / 2
      children.forEach(child => {
        process(child.id, x, currentY)
        currentY += gap
      })
    }
    process(rootId, x, startY)
    return positions
  }

  const root = subtree.find(n => n.parentId === null)
  if (!root) return
  const positions = layoutSubtree(root.id, 200, 80)

  treeFlowNodes.value = subtree.map(node => ({
    id: node.id,
    type: 'tree-node',
...
const currentTreeProgress = computed(() => calcTreeProgress(currentSubtree.value))

const fitTreeView = () => rebuildTreeFlow()

// 当导入新数据后，如果当前章节不存在或不合适，自动切换到第一个根节点
watch(localTree, (newTree) => {
  const chapters = newTree.filter(n => n.parentId === null)
  if (chapters.length > 0 && !chapters.find(c => c.id === currentChapterId.value)) {
...
const addRootNodeIfEmpty = () => {
  // 在当前章节下添加根节点，如果当前没有选章，则创建新的一级标题
  if (currentChapterId.value) {
    const exists = localTree.value.some(n => n.parentId === null && n.id === currentChapterId.value)
    if (exists) {
      ElMessage.warning('当前章节根节点已存在，可直接在其下添加子节点')
      return
    }
  }
  // 创建新根
  ...
  localTree.value.push(newRoot)
  localTree.value = [...localTree.value]
  currentChapterId.value = newRoot.id
  rebuildTreeFlow()
}
...
const deleteNode = (nodeId) => {
  ...
  ElMessageBox.confirm(`确定删除该节点及其 ${idsToDelete.length - 1} 个子节点吗？`, { type: 'warning' }).then(() => {
    ...
    if (currentChapterId.value === nodeId) {
...
.chapter-card { background: #f8fafc; }
.chapter-info { margin-top: 8px; font-size: 13px; color: #64748b; }
