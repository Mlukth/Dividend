  ...
  <div class="tree-editor-page">
    ...
    <div class="left-panel">
      <div class="panel-header">
        <h2>{{ taskName }} - 决策树</h2>
        <el-button type="warning" size="small" @click="showPromptEditor = true">
          ⚙️ 提示词模板
        </el-button>
      </div>
...
const calculateTreeLayout = (nodes) => {
  ...
  for (const depth of Object.keys(levelGroups).sort((a, b) => a - b)) {
    ...
    ids.sort((a, b) => {
      const pa = nodeMap[a]?.parentId, pb = nodeMap[b]?.parentId
      if (pa === pb) {
        const siblings = nodes.filter(n => n.parentId === pa)
    ...
    ids.forEach((id, index) => {
      positions[id] = { x: startX + index * horizontalSpacing, y: depth * verticalSpacing + 60 }
    })
  }
  return positions
}
