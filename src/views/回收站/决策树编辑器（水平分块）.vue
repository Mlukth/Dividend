...
// ---------- 水平分块布局算法 ----------
const calculateHorizontalLayout = (nodes) => {
  ...
  // 找到所有根节点（parentId为null 或 父节点不在数据中）
  const rootNodes = nodes.filter(n => n.parentId === null || !nodeMap[n.parentId])

  const positions = {}
  let rootY = 50
  const verticalSpacing = 300  // 每个“块”之间的垂直间距
  const horizontalSpacing = 280  // 父子节点之间的水平间距

  // 为每棵子树计算位置（根在左侧，子节点向右扩展）
  const layoutSubtree = (nodeId, x, y) => {
    positions[nodeId] = { x, y }
    const children = childrenMap[nodeId] || []
    let childY = y - (children.length - 1) * 80 / 2  // 子节点垂直居中
    children.forEach((child, index) => {
      layoutSubtree(child.id, x + horizontalSpacing, childY + index * 80)
    })
  }

  rootNodes.forEach(root => {
    layoutSubtree(root.id, 80, rootY)
    rootY += verticalSpacing
  })
