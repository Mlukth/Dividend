      ...
      <div class="ett-header-actions">
        ...
        <el-select v-model="currentEssayId" placeholder="选择练习篇目" filterable size="small" style="width:260px">
          <el-option v-for="(e, idx) in essays" :key="e.id" :label="`${e.date} ${e.title}`" :value="e.id">
            <span>{{ e.title }}</span>
            <span style="float:right;color:#94a3b8;font-size:12px">{{ e.date }}</span>
          </el-option>
        </el-select>
...
<!-- 翻译输入区 -->
      ...
      <aside class="ett-right">
        <template v-if="scoredRecord">
          ...
          <!-- 水波纠错展示 -->
          <div v-if="scoredRecord?.mistakeWave?.patternEN && scoredRecord.mistakeWave.patternEN !== '无'">
            <div class="feedback-title">🌊 错误结构分析</div>
            <div class="mw-block" v-if="scoredRecord.mistakeWave.patternEN">
              <div class="mw-label">卡住你的英文结构</div>
              <p class="mw-pattern">{{ scoredRecord.mistakeWave.patternEN }}</p>
            </div>
            <div class="mw-block" v-if="scoredRecord.mistakeWave.whereStuck">
              <div class="mw-label">为什么容易卡</div>
              <p class="mw-text">{{ scoredRecord.mistakeWave.whereStuck }}</p>
            </div>
            <div class="mw-block" v-if="scoredRecord.mistakeWave.examples?.length">
              <div class="mw-label">同类例句（英→中）</div>
              <div v-for="(ex, i) in scoredRecord.mistakeWave.examples" :key="i" class="mw-ex-row">
                <span class="mw-ex-en">{{ ex.en }}</span>
                <span class="mw-ex-arrow">→</span>
                <span class="mw-ex-zh">{{ ex.zh }}</span>
              </div>
            </div>
            <div class="mw-block" v-if="scoredRecord.mistakeWave.nextTime">
              <div class="mw-label">下次遇到怎么拆</div>
              <p class="mw-text">{{ scoredRecord.mistakeWave.nextTime }}</p>
            </div>
          </div>
    ...
    <!-- 生词池全量查看对话框 -->
    <el-dialog v-model="showVocabPoolDialog" title="生词短语池" width="720px" destroy-on-close>
    ...
    <el-dialog v-model="showImageImportDialog" title="图片导入翻译题" width="800px" destroy-on-close @opened="onImageDialogOpened">
      ...
      <div v-else class="batch-import-layout">
        <div class="batch-import-left">
          <div class="image-import-label">逐张粘贴区（点击后 Ctrl+V 粘贴，每张一道题）</div>
...
async function loadData() {
  ...
  // 首次迁移：localStorage 为空时从服务端旧数据读取
  if (!loaded) {
    try {
      const res = await fetch('/api/ett-data')
      if (res.ok) {
        const data = await res.json()
        if (data.essays?.length || data.records?.length) {
          essays.value = data.essays || []
          records.value = data.records || []
          essayOrder.value = data.essayOrder || []
          essayAnnotations.value = data.annotations || {}
          ...
          try {
            ...
            if (wcRes.ok) waveCache.value = await wcRes.json()
          } catch {}
          syncData()
          loaded = true
        }
...
async function copyAllBatchPrompt() {
  ...
  try {
    ...
    const clipboardItem = new ClipboardItem({
      'text/plain': new Blob([promptWithHint], { type: 'text/plain' }),
      'image/png': imageBlob
    })
    await navigator.clipboard.write([clipboardItem])
