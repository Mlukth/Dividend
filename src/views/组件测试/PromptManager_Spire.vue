...
const checkQwenStatus = async () => {
  ...
  try {
    ...
    const res = await fetch(`${QWEN_API}/api/tags`, { signal: ctrl.signal, mode: 'cors' })
...
// === 修复4：优化 Qwen 标题生成提示词，移除默认标题覆盖问题 ===
const generateTitleByQwen = async (content, currentTitle) => {
  // 检测内容类型
  const isCode = /function|class|import|export|const|let|var|<template|<script|<style|def |public |private /i.test(content)
  const hasTitle = /^[\s#]*[A-Za-z\u4e00-\u9fa5]{2,30}[\n:：]/.test(content)
  
  const prompt = `你是专业的内容标题生成专家。请为以下内容生成一个准确、简洁的中文标题。

【要求】
1. 标题长度：8-15个汉字（或等长英文）
2. 标题特征：
   - 准确反映内容核心主题
   - 避免"关于"、"介绍"等泛词
   - 使用名词短语或动宾结构
   - 具有信息量和区分度
3. 只返回标题本身，不要加引号、序号或其他说明

【内容类型】${isCode ? '代码/技术文档' : hasTitle ? '可能已有标题的笔记' : '普通文本/笔记'}
【当前标题】"${currentTitle}"（仅供参考，如果质量差请重新生成更好的）
  ...
  try {
    ...
    const res = await fetch(`${QWEN_API}/api/generate`, {
      ...
      body: JSON.stringify({
        ...
        prompt,
        stream: false,
        options: { temperature: 0.5, num_predict: 50, top_p: 0.9 }
      }),
      signal: ctrl.signal,
      mode: 'cors'
    })
    ...
    console.log('[Qwen] 清理后标题:', title)
    
    // 验证标题质量 - 放宽条件
    if (!title || title.length < 3) {
      console.warn('[Qwen] 标题过短或为空')
      return null
    }
    
    // 不再检查和默认标题是否一样，只要Qwen返回了有效标题就使用
    // if (title === makeTitle(content)) return null // 移除这个检查
    
    return title
  } catch (e) {
    console.warn('[Qwen] 标题生成失败:', e.message)
    return null
  }
}
...
const scanUnmarkedCards = async () => {
  ...
  for (let i = 0; i < toScan.length; i++) {
    ...
    await new Promise(r => setTimeout(r, 500)) // 增加延迟避免请求过快
  }
