  ...
  <div class="spire-app">
    ...
    <div class="spire-header">
      ...
      <div class="header-stats">
        ...
        <span>提示词 {{ promptMode ? '已注入' : '未启用' }}</span>
        <span>复制 {{ copyMode ? '就绪' : '浏览' }}</span>
      </div>
    </div>
    ...
    <div class="spire-main">
      <div class="spire-toolbar">
        <h2 class="spire-title">卡组管理</h2>
        <div class="toolbar-actions">
          <!-- 纯CSS开关 -->
          <label class="spire-switch" title="开启后点击卡片将自动拼接提示词">
            <input type="checkbox" v-model="promptMode">
            <span class="spire-slider"></span>
            <span class="spire-switch-label">提示词注入</span>
          </label>
          <label class="spire-switch" title="开启后点击卡片直接复制到剪贴板">
            <input type="checkbox" v-model="copyMode">
            <span class="spire-slider"></span>
            <span class="spire-switch-label">复制模式</span>
          </label>
          <button class="spire-btn-ghost" @click="exportData">📤 导出卡组</button>
      ...
      <div class="spire-inputs">
        <div class="spire-input-group">
          <label>📜 提示词模板 <span v-if="promptMode" class="spire-badge active">已激活</span></label>
          <textarea v-model="promptTemplate" :disabled="!promptMode" 
            placeholder="在此输入系统提示词/上下文规则..."></textarea>
        </div>
        <div class="spire-input-group">
          <label>🔥 待锻造内容</label>
      ...
      <div class="spire-grid">
        <div v-for="card in cards" :key="card.id" 
             :class="['spire-card', card.type, card._rarity, { 'is-copyable': copyMode }]"
          ...
          <div class="type-banner" :class="card.type">
            <span>{{ card.cat2Label || card.type }}</span>
          </div>
          <!-- 描述框 -->
          <div class="desc-box">
          ...
          <!-- 复制提示浮层 -->
          <div v-if="copyMode" class="spire-copy-overlay">📋 点击复制</div>
        </div>
...
// === 持久化存储 ===
const STORAGE_KEY = 'spire_prompt_cards_v1'
...
const handleCardClick = async (card) => {
  ...
  const finalText = promptMode.value
  ...
  try {
    ...
    showToast(`📋 已复制${promptMode.value ? ' (含提示词)' : ''}`, 'success')
  } catch (err) {
...
const exportData = () => {
  ...
  const blob = new Blob([JSON.stringify(cards.value, null, 2)], { type: 'application/json' })
...
const clearData = () => {
  if (confirm('确定要清空本地所有卡牌吗？此操作不可撤销。')) {
    cards.value = []
    localStorage.removeItem(STORAGE_KEY)
    showToast('🗑️ 卡组已清空', 'success')
  }
}
...
/* 纯CSS开关 */
.spire-switch { display: flex; align-items: center; gap: 8px; cursor: pointer; user-select: none; font-size: 12px; color: #8a7a6a; }
.spire-switch input { display: none; }
.spire-slider {
  width: 36px; height: 18px; background: #2a1f14; border-radius: 18px; position: relative; transition: 0.2s;
  border: 1px solid #3a2a1a;
}
.spire-slider::after {
  content: ''; position: absolute; top: 1px; left: 1px; width: 14px; height: 14px;
  background: #6a5a4a; border-radius: 50%; transition: 0.2s;
}
.spire-switch input:checked + .spire-slider { background: rgba(200,164,92,0.2); border-color: #c8a45c; }
.spire-switch input:checked + .spire-slider::after { left: 19px; background: #c8a45c; box-shadow: 0 0 6px rgba(200,164,92,0.4); }
...
.spire-badge.active { font-size: 9px; background: rgba(200,164,92,0.15); color: #c8a45c; padding: 2px 5px; border-radius: 3px; }
textarea {
...
textarea:disabled { background: #0d0805; color: #4a3a2a; cursor: not-allowed; border-color: #1a1108; }
...
.spire-card.is-copyable { cursor: pointer; }
.spire-card.is-copyable:hover {
...
.spire-card.is-copyable:hover.copper { border-color: #6a4a2a; }
.spire-card.is-copyable:hover.bronze { border-color: #8b5a2b; }
.spire-card.is-copyable:hover.silver { border-color: #a0a0a0; }
.spire-card.is-copyable:hover.gold   { border-color: #c8a040; }
...
.spire-card.is-copyable:hover .spire-copy-overlay { transform: translateY(0); }
