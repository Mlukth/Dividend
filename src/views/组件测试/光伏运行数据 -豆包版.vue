      ...
      <div class="gauge-card-large">
        <div class="gauge-wrapper">
          ...
          <div class="power-value">
            <div class="power-capacity">组件总容量：2.336 MWp</div>
          </div>
        </div>
      </div>
...
const initGauge = () => {
  ...
  gaugeChart.setOption({
    ...
    series: [{
      ...
      title: { show: false },
      detail: { fontSize: 28, fontWeight: 700, color: '#ff6b00', offsetCenter: [0, '60%'], formatter: '{value} kW' },
      data: [{ value: 42.84 }]
    }]
  })
}
...
/* ==================== 第一行：左右视觉融合为1个整体卡片 ==================== */
.row-top { display: flex; gap: 0; margin-bottom: 20px; }
.merged-card-box {
  background: #fff;
  border-radius: 16px;
...
.chart-container { width: 100%; height: 580px; }
...
/* 温度监测 */
.temp-card { flex: 1.5; background:#fff; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.06); }
...
/* 环境监测 */
.env-card { flex: 1; background:#fff; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.06); }
...
/* 电站评估 */
.eval-card { flex: 0.8; background:#fff; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.06); }
...
.stat-number { font-size: 28px; font-weight: 700; color: #52c41a; line-height: 1.2; }
