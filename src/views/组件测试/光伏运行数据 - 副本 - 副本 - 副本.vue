        ...
        <div class="gauge-content">
          ...
          <div class="gauge-info">
            <div class="info-item main-value">
              <div class="info-label">当前功率</div>
              <div class="info-number">42.84 <span class="unit">kW</span></div>
            </div>
            <div class="info-item">
              <div class="info-label">组件总容量</div>
              <div class="info-number small">2.336 <span class="unit">MWp</span></div>
            </div>
            <div class="info-item">
              <div class="info-label">实时功率占比</div>
              <div class="info-number small">1.83 <span class="unit">%</span></div>
            </div>
          </div>
        </div>
      </div>
...
const initGauge = () => {
  ...
  gaugeChart.setOption({
    ...
    series: [{
      ...
      progress: { show: true, width: 18, itemStyle: { color: deepColor } },
      pointer: { length: '60%', width: 5, itemStyle: { color: '#ff6b00' } },
      anchor: { size: 12, itemStyle: { color: '#ff6b00' } },
      title: { show: false },
      detail: { fontSize: 28, fontWeight: 700, color: '#ff6b00', offsetCenter: [0, '60%'], formatter: '{value}%' },
      data: [{ value: 32 }]
    }]
  })
}
...
/* ==================== 卡片通用 ==================== */
.gauge-card-large, .kpi-card, .chart-card, .temp-card, .env-card, .eval-card, .social-card, .layout-card {
  background: #fff;
  border-radius: 16px;
  padding: 24px;
...
/* 实时功率大卡片 */
.gauge-card-large { flex: 0 0 400px; }
.gauge-content { display: flex; align-items: center; gap: 24px; }
.gauge-main { flex: 1; }
.gauge-chart { width: 220px; height: 220px; }
.gauge-info { flex: 1; display: flex; flex-direction: column; gap: 20px; }
.info-item { }
.info-item.main-value .info-label { font-size: 16px; color: #888; margin-bottom: 8px; }
.info-item.main-value .info-number { font-size: 42px; font-weight: 700; color: #ff6b00; }
.info-item.main-value .unit { font-size: 18px; font-weight: 400; color: #999; margin-left: 6px; }
.info-label { font-size: 14px; color: #888; margin-bottom: 6px; }
.info-number.small { font-size: 28px; font-weight: 700; color: #1a1a2e; }

/* KPI 2x2 */
.kpi-card { flex: 1; }
.kpi-grid { display: grid; grid-template-columns: repeat(2, 1fr); grid-template-rows: repeat(2, 1fr); gap: 16px; height: calc(100% - 50px); }
.kpi-item { padding: 20px; background: #f8f9fb; border-radius: 12px; display: flex; flex-direction: column; justify-content: center; transition: all 0.3s ease; }
...
.kpi-item.highlight .kpi-label, .kpi-item.highlight .kpi-value, .kpi-item.highlight .kpi-sub { color: #fff; }
.kpi-label { font-size: 14px; color: #888; margin-bottom: 8px; }
...
.kpi-sub { font-size: 13px; color: #666; margin-top: 6px; }
...
.social-card { flex: 0 0 400px; }
.social-stats { display: flex; justify-content: space-around; padding: 10px 0; }
...
.layout-card { flex: 1; }
.layout-placeholder { height: 100px; background: linear-gradient(135deg, #f8f9fb, #e8ecf0); border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #999; font-size: 16px; }

/* ==================== 响应式 ==================== */
@media screen and (max-width: 1400px) {
  .row-top { flex-wrap: wrap; }
  .gauge-card-large { flex: 0 0 100%; }
...
@media screen and (max-width: 768px) {
  ...
  .gauge-card-large, .kpi-card, .temp-card, .env-card, .eval-card, .social-card, .layout-card { flex: 1 1 100%; }
