    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <el-breadcrumb separator="/" class="breadcrumb">
          ...
          <el-breadcrumb-item>电站档案信息</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="title-decoration-line"></div>
      </div>
      <div class="header-right">
        <el-button class="action-btn" :icon="Printer">打印预览</el-button>
        <el-button type="primary" class="action-btn primary-btn" :icon="Download">导出档案</el-button>
      </div>
    </div>
              ...
              <div class="attr-info">
                ...
                <div class="attr-value">
                  <div class="voltage-bar">
                    <div class="v-bar" :style="{ height: voltageHeight + '%' }"></div>
    ...
    <el-card class="module-card" shadow="hover">
      <template #header>
        <div class="card-header with-actions">
          ...
          <div class="header-toolbar">
            <div class="toolbar-left">
              <el-select v-model="docTypeFilter" placeholder="图纸类型" clearable style="width: 140px;">
                <el-option label="全部" value="" />
                <el-option label="电气原理图" value="电气原理图" />
                <el-option label="平面布置图" value="平面布置图" />
                <el-option label="结构施工图" value="结构施工图" />
              </el-select>
            </div>
        ...
        <el-table :data="filteredDrawings" style="width: 100%" class="custom-table">
          <el-table-column label="缩略图" width="140">
            <template #default="{ row }">
              <div class="thumb-wrap" @click="openPreview(row)">
                <div class="thumb-placeholder" :style="{ background: row.thumbBg }">
          ...
          <el-table-column prop="updateDate" label="更新日期" width="120">
            <template #default="{ row }">
              <div class="date-cell">
                {{ row.updateDate }}
                <span v-if="row.isNew" class="new-badge">NEW</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="180" fixed="right">
            <template #default="{ row }">
              <el-link :underline="false" style="margin-right: 8px;" @click="openPreview(row)"><el-icon><View /></el-icon> 预览</el-link>
              <el-link :underline="false" @click="downloadDrawing(row)"><el-icon><Download /></el-icon> 下载</el-link>
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <div v-else class="grid-view">
        <div v-for="(doc, idx) in filteredDrawings" :key="idx" class="doc-card" @click="openPreview(doc)">
          <div class="doc-card-thumb" :style="{ background: doc.thumbBg }">
      ...
      <div class="preview-content">
        ...
        <div class="preview-sidebar">
          ...
          <div class="info-row">
            ...
            <span>{{ currentDrawing?.version }}</span>
          </div>
          <div class="info-row">
...
const drawings = ref([
  { name: '电气主接线图', type: '电气原理图', id: 'RY-SG-EL-001', version: 'v2.1', isLatest: true, updateDate: '2024-01-10', isNew: true, thumbBg: 'linear-gradient(135deg, #E0F2FF, #B3D8FF)' },
  { name: '总平面布置图', type: '平面布置图', id: 'RY-SG-PL-002', version: 'v1.5', isLatest: true, updateDate: '2023-11-22', isNew: false, thumbBg: 'linear-gradient(135deg, #E8FFEA, #B3F2B8)' },
  { name: '基础结构施工图', type: '结构施工图', id: 'RY-SG-ST-003', version: 'v3.0', isLatest: true, updateDate: '2024-02-01', isNew: false, thumbBg: 'linear-gradient(135deg, #FFF7E0, #FFE8A8)' }
])
...
const filteredDrawings = computed(() => {
  return drawings.value.filter(item => {
    return !docTypeFilter.value || item.type === docTypeFilter.value
  })
})
...
.v-bar {
  width: 16px;
  background: linear-gradient(180deg, #0066CC, #165DFF);
