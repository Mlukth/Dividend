  ...
  <div class="container">
    ...
    <div class="main-content">
      ...
      <div class="left-panel">
        ...
        <!-- 最近操作 -->
        <div class="card" v-if="lastRecord">
          <div class="card-title">
...
const members = ref([
  ...
  { id: 'member_03', name: '张嘉凌', totalPoints: 0 },
  { id: 'member_04', name: '赵春红', totalPoints: 0 },
...
const exportExcel = () => {
  ...
  // Sheet 2: 原始操作日志
  const sheet2Data = [['日期', '成员', '任务名称', '难度', '工时', '质量系数', '积分', '时间']]
  records.value.forEach(r => {
    sheet2Data.push([
      r.date,
      r.memberName,
...
const initPieChart = () => {
  // 需要引入 echarts 库
  if (typeof echarts === 'undefined' || !pieChartRef.value) return
  ...
  const option = {
    ...
    series: [{
      ...
      data: members.value.map((m, index) => ({
        ...
        name: m.name,
        itemStyle: {
          color: ['#667eea', '#764ba2', '#f093fb', '#f5576c', '#4facfe'][index]
        }
      }))
    }]
  }
