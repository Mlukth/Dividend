  ...
  <div class="inverter-card-grid-container">
    ...
    <div class="station-header">
      ...
      <div class="header-actions">
        <button class="btn btn-outline" @click="toggleViewMode" :class="{ active: currentView === 'table' }">
          <span class="btn-icon">{{ currentView === 'table' ? '📱' : '📊' }}</span>
          {{ currentView === 'table' ? '卡片视图' : '表格视图' }}
        </button>
    ...
    <!-- ==================== 卡片网格视图 ==================== -->
    <div v-if="currentView === 'card'" class="cards-container" :class="[layoutMode, `size-${cardSize}`]">
      <div
    ...
    <div v-if="currentView === 'table'" class="table-container">
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th class="th-model">设备型号</th>
              <th class="th-sn">SN编号</th>
              <th class="th-area">安装区域</th>
              <th class="th-rated">额定功率<br/>(kW)</th>
              <th class="th-realtime">实时功率<br/>(kW)</th>
              <th class="th-util">利用率<br/>(%)</th>
              <th class="th-daily">当日电量<br/>(kWh)</th>
              <th class="th-cumulative">累计电量<br/>(kWh)</th>
              <th class="th-hours">当日等效<br/>(h)</th>
              <th class="th-total-hours">累计等效<br/>(h)</th>
              <th class="th-trend">功率趋势<br/>(%)</th>
              <th class="th-commission">投产日期</th>
              <th class="th-warranty">质保到期</th>
              <th class="th-timestamp">数据时间戳</th>
          ...
          <tbody>
            <tr
              ...
              class="table-row"
              :class="{
                'row-selected': isSelected(inverter.id),
                'row-fault': inverter.status === 'fault',
                'row-stopped': inverter.status === 'stopped'
              }"
            >
              <td class="td-checkbox">
                <input
                  type="checkbox"
                  :checked="isSelected(inverter.id)"
                  @change="toggleSelect(inverter.id)"
                  class="checkbox-input"
                />
              </td>
              <td class="td-status">
                <span class="status-badge-cell" :class="`status-${inverter.status}`">
                  <span class="status-dot-cell"></span>
                  {{ statusText(inverter.status) }}
                </span>
              </td>
              <td class="td-name">
                <div class="name-cell">
                  <div class="name-title">{{ inverter.name }}</div>
                  <div class="name-sn">{{ inverter.sn }}</div>
                </div>
              </td>
              <td class="td-model">
                <span class="model-cell">{{ inverter.model }}</span>
              </td>
              <td class="td-sn">
                <span class="sn-cell">{{ inverter.sn }}</span>
              </td>
              <td class="td-area">
                <span class="area-cell">{{ inverter.area }}</span>
              </td>
              <td class="td-rated">
                <span class="rated-cell">{{ inverter.ratedPower }}</span>
              </td>
              <td class="td-realtime">
                <div class="realtime-cell">
                  <span class="power-value-cell" :class="getTrendClass(inverter.powerTrend)">
                    {{ inverter.realTimePower.toFixed(1) }}
                  </span>
                  <div class="power-mini-bar">
                    <div 
                      class="power-fill" 
                      :style="{ width: (inverter.realTimePower / inverter.ratedPower) * 100 + '%' }"
                    ></div>
                  </div>
                </div>
              </td>
              <td class="td-util">
                <div class="util-cell">
                  <span class="util-value-cell" :class="getUtilizationClass(inverter.utilizationRate)">
                    {{ inverter.utilizationRate.toFixed(1) }}%
                  </span>
                  <div class="util-bar-cell">
                    <div 
                      class="util-fill-cell" 
                      :class="getUtilizationClass(inverter.utilizationRate)"
                      :style="{ width: inverter.utilizationRate + '%' }"
                    ></div>
                  </div>
              ...
              <td class="td-daily">
                <span class="daily-cell">{{ inverter.dailyEnergy.toFixed(0) }}</span>
              </td>
              <td class="td-cumulative">
                <span class="cumulative-cell">{{ formatLargeNumber(inverter.cumulativeEnergy) }}</span>
              </td>
              <td class="td-hours">
                <span class="hours-cell">{{ inverter.equivalentHours.toFixed(1) }}</span>
              </td>
              <td class="td-total-hours">
                <span class="total-hours-cell">{{ inverter.cumulativeEquivalentHours.toFixed(1) }}</span>
              </td>
              <td class="td-trend">
                <span class="trend-cell" :class="getTrendClass(inverter.powerTrend)">
                  {{ inverter.powerTrend >= 0 ? '↑' : '↓' }} {{ Math.abs(inverter.powerTrend).toFixed(1) }}%
                </span>
              </td>
              <td class="td-commission">
                <span class="commission-cell">{{ formatDate(inverter.commissioningTime) }}</span>
              </td>
              <td class="td-warranty">
                <span class="warranty-cell" :class="getWarrantyClass(inverter.warrantyExpiration)">
                  {{ formatDate(inverter.warrantyExpiration) }}
                </span>
              </td>
              <td class="td-timestamp">
                <span class="timestamp-cell">{{ formatTimestampShort(inverter.dataTimestamp) }}</span>
              </td>
              <td class="td-updated">
                <span class="updated-cell">{{ formatTimestampShort(inverter.updateTime) }}</span>
              </td>
              <td class="td-actions">
                <div class="actions-cell">
                  <button class="table-action-btn" @click="viewDetail(inverter)" title="查看详情">
                    📋
                  </button>
                  <button class="table-action-btn" @click="refreshSingle(inverter)" title="刷新数据">
                    🔄
                  </button>
                  <button 
                    class="table-action-btn" 
                    @click="controlDevice(inverter)"
                    :title="inverter.status === 'stopped' ? '启动设备' : '停机'"
                  >
                    {{ inverter.status === 'stopped' ? '▶️' : '⏸️' }}
                  </button>
                </div>
              </td>
            </tr>
        ...
        <!-- 表格空状态 -->
        <div v-if="filteredInverters.length === 0" class="empty-table">
          <div class="empty-icon">📭</div>
          <div class="empty-title">没有找到匹配的设备</div>
          <button class="empty-btn" @click="resetFilters">
            清除所有筛选
...
const generateInverters = () => {
  ...
  for (let i = 1; i <= 48; i++) {
    ...
    data.push({
      ...
      cumulativeEquivalentHours: Math.random() * 3000 + 500,
      ...
      dataTimestamp: Date.now() - Math.random() * 600000,
      updateTime: Date.now() - Math.random() * 300000
    })
  }
...
const isAllSelected = computed(() => {
  return paginatedInverters.value.length > 0 && 
         paginatedInverters.value.every(i => selectedInverters.value.includes(i.id))
})
...
const toggleSelectAll = () => {
  if (isAllSelected.value) {
    paginatedInverters.value.forEach(i => {
      const index = selectedInverters.value.indexOf(i.id)
      if (index > -1) selectedInverters.value.splice(index, 1)
    })
  } else {
    paginatedInverters.value.forEach(i => {
      if (!selectedInverters.value.includes(i.id)) {
        selectedInverters.value.push(i.id)
      }
    })
  }
}
...
const refreshSingle = (inverter) => {
  ...
  inverter.dataTimestamp = Date.now()
  inverter.updateTime = Date.now()
...
const toggleViewMode = () => {
  currentView.value = currentView.value === 'card' ? 'table' : 'card'
}
  ...
  setInterval(() => {
    ...
    for (let i = 0; i < refreshCount; i++) {
      ...
      if (inv && inv.status === 'running') {
        ...
        inv.dataTimestamp = Date.now()
        inv.updateTime = Date.now()
      }
    }
  }, 30000)
})
...
.status-badge-cell.status-running {
  background: #E8FFEA;
  color: #00B42A;
}

.status-badge-cell.status-fault {
  background: #FFECE8;
  color: #D91A2C;
}

.status-badge-cell.status-stopped {
  background: #F7F8FA;
  color: #86909C;
}
...
.power-value-cell.trend-up {
  color: #00B42A;
}

.power-value-cell.trend-down {
  color: #D91A2C;
}
...
.util-value-cell.util-high {
  color: #00B42A;
}

.util-value-cell.util-medium {
  color: #0066CC;
}

.util-value-cell.util-low {
  color: #FF7D00;
}
...
.util-fill-cell.util-high {
  background: linear-gradient(90deg, #00B42A, #00C975);
}

.util-fill-cell.util-medium {
  background: linear-gradient(90deg, #0066CC, #165DFF);
}

.util-fill-cell.util-low {
  background: linear-gradient(90deg, #FF7D00, #FFA94D);
}
...
.trend-cell.trend-down {
  color: #D91A2C;
}

.warranty-cell.text-warning {
  color: #FF7D00;
  font-weight: 500;
}

.warranty-cell.text-danger {
  color: #D91A2C;
  font-weight: 500;
}
