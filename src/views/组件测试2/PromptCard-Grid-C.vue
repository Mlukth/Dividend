  ...
  <div class="prompt-card-container">
    ...
    <header class="relic-bar">
      <div class="relic-left">
        ...
        <span class="scheme-badge scheme-c">方案C - 局部金属</span>
      </div>
    ...
    <div class="card-grid">
      ...
      >
        <!-- 复杂度球 - 铜级用金属质感 -->
        <div :class="['complexity-sphere', `complexity-${card.complexity}`, `rarity-${card.rarity}`]">
          {{ card.complexity }}
        </div>
        ...
        <div class="card-content">
          ...
          <div class="card-version">
            <span :class="['version-text', `version-${card.rarity}`]">v{{ card.version }}</span>
            <span v-if="card.variants > 0" :class="['variant-badge', `badge-${card.rarity}`]">+{{ card.variants }}</span>
          </div>
        </div>
      </div>
    </div>
...
.scheme-c { background: rgba(169,113,66,0.2); color: #A97142; border-color: rgba(169,113,66,0.3); }
...
/* 铜级复杂度球 - 金属质感 */
.complexity-sphere.rarity-copper {
  background: linear-gradient(145deg, #CB6A52 0%, #A0522D 50%, #8B4513 100%);
  border: 1px solid rgba(203,106,82,0.5);
  color: #fff;
  box-shadow: 
    inset 0 2px 4px rgba(255,255,255,0.3),
    inset 0 -2px 4px rgba(0,0,0,0.3),
    0 2px 8px rgba(203,106,82,0.3);
  text-shadow: 0 1px 2px rgba(0,0,0,0.3);
}

/* 青铜级复杂度球 */
.complexity-sphere.rarity-bronze {
  background: linear-gradient(145deg, #B08D57 0%, #A97142 50%, #6E3A06 100%);
  border-color: rgba(169,113,66,0.6);
  color: #fff;
  box-shadow: inset 0 2px 4px rgba(255,255,255,0.25), inset 0 -2px 4px rgba(0,0,0,0.3);
}

/* 银级复杂度球 */
.complexity-sphere.rarity-silver {
  background: linear-gradient(145deg, #D4D8DC 0%, #B4B9BE 50%, #8B9094 100%);
  border-color: rgba(180,185,190,0.6);
  color: #333;
  box-shadow: inset 0 2px 4px rgba(255,255,255,0.5), inset 0 -2px 4px rgba(0,0,0,0.2);
}

/* 金级复杂度球 */
.complexity-sphere.rarity-gold {
  background: linear-gradient(145deg, #FBF8AE 0%, #C5AB50 50%, #AC933E 100%);
  border-color: rgba(197,171,80,0.6);
  color: #1a0e00;
  box-shadow: inset 0 2px 4px rgba(255,255,255,0.5), inset 0 -2px 4px rgba(0,0,0,0.2), 0 0 12px rgba(197,171,80,0.4);
}

/* 版本标签 - 铜级金属效果 */
.version-text.rarity-copper {
  background: linear-gradient(135deg, #CB6A52 0%, #A0522D 100%);
  padding: 2px 6px;
  border-radius: 3px;
  color: #fff;
  font-weight: 600;
  text-shadow: 0 1px 1px rgba(0,0,0,0.2);
  box-shadow: 0 1px 3px rgba(203,106,82,0.3);
}

/* 变体徽章 - 铜级金属效果 */
.variant-badge.badge-copper {
  background: linear-gradient(135deg, #CB6A52 0%, #A0522D 100%);
  box-shadow: 0 1px 3px rgba(203,106,82,0.4);
}
